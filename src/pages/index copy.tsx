...
const Index = ({ PAGE_HEIGHT, PAGE_WIDTH }: Props) => {
    ...
    const changePage = async (page_number: number) => {
        controls.start({
            x: (-1) * PAGE_WIDTH * page_number,
            transition: TRANSITION_CONFIG
        });
        await page_underline.start({
            x: PAGE_WIDTH / PAGE_COUNT * page_number,
            transition: TRANSITION_CONFIG
        })
    }
        ...
        <AnimatePresence>
            ...
            <motion.div style={{ width: PAGE_WIDTH, height: PAGE_HEIGHT }} className="relative bg-gray-200 mx-auto my-auto overflow-hidden  flex flex-col">
                <motion.div
                    ...
                    animate={pageDrawer}
                    className=" flex flex-row- justify-start  absolute z-50 left-0 transform -translate-x-full bg-opacity-25 bg-white">
                    <motion.div
                        // drag="x"
                ...
                <motion.div
                    ...
                    style={{ width: PAGE_WIDTH, height: PAGE_HEIGHT, transform: `translateX(${PAGE_WIDTH})` }}
                    className=" flex flex-col justify-start  absolute z-50  transform   bg-opacity-25 bg-white">
                    <motion.div style={{ width: PAGE_WIDTH, height: PAGE_HEIGHT }} className="  flex-1" onTap={closeBottomSheet} />
                    <motion.div
                        drag="y"
                        style={{
                    ...
                    >
                        <div style={{ height: '7rem', width: PAGE_WIDTH }} className="flex flex-col   justify-center items-center  border-b-2 border-gray-800">
                        ...
                        <div style={{ width: PAGE_WIDTH }} className="h-24 flex flex-col items-stretch justify-around">
                        ...
                        <div style={{ width: PAGE_WIDTH }} className=" px-6 py-3 justify-center h-16 flex items-center flex-col">
                            <button onClick={closeBottomSheet} style={{ width: PAGE_WIDTH, height: "3rem" }} className=" rounded-full text-center align-middle bg-gray-800 cursor-pointer font-bold flex justify-center items-center text-white ">
                                <span>cancel</span>
                            </button>
                        </div>
                    </motion.div>
                ...
                {modal.show ? (<motion.div
                    initial={false}
                    animate={modal.show ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0 }}
                    exit={{ opacity: 0, scale: 0, transition: { duration: 0.15 } }}
                    transition={{ duration: 0.2, delay: 0.15 }}
                    style={{ pointerEvents: "auto", width: PAGE_WIDTH, height: PAGE_HEIGHT }}
                ...
                <nav className="w-full h-12 flex flex-row items-stretch relative border-t-2 border-gray-700 bg-gray-900">
                    ...
                    {/* bottom bar */}
                    <motion.div animate={page_underline} style={{ width: PAGE_WIDTH / PAGE_COUNT }} transition={DEFAULT_TRANSION} className="absolute bottom-0 left-0 rounded-full mb-1  h-1 bg-red-600"> </motion.div>
                </nav>
