...
const TweetTop = ({ tweet_text, user_image, user_name, user_id, tweet_date, tweet_image, onTap, onImageTap }: Props) => {
            ...
            <motion.div {...{ onTap }} whileHover={{ cursor: "pointer", backgroundColor: 'rgb(35, 45, 62)' }} variants={item} layout layoutId={user_id + user_image} className="w-full border-b border-gray-700 px-3  pt-2 text-white">
                ...
                <div className="flex flex-col my-1 justify-start items-stretch  text-sm text-white">
                    <div className="w-full flex items-stretch flex-row  content-start pb-3">
                        <div className=" w-12 rounded-full overflow-hidden text-right"><img className="w-10 rounded-full " src={user_image} width={"100%"} alt="user icon" /></div>
                        <div className="text-left text-sm flex flex-col ">
                            <div className="mr-2 capitalize">{user_name}</div>
                            <div className="mr-2 text-gray-700 text-sm underline">@{user_id} </div>
                            {/* <span className="text-gray-700">{tweet_date}</span> */}
                        </div>
                        <div style={{ justifySelf: "end" }} className="self-center flex flex-1 justify-end text-right "> <FiChevronDown className="text-right" /></div>
                    </div>
                    <div className="flex-1 flex flex-col">
                        {/* user name */}

                        <div className="text-left mb-2">
                            <span className="text-base">{tweet_text + tweet_text + tweet_text + tweet_text + tweet_text}</span>
                        </div>
                        {tweet_image && (
                            <motion.div
                                onClick={onImageTap}
                        ...
                        <div className="w-full  text-gray-700  text-xs py-2 border-t border-b border-gray-800 font-semibold">
                            <span>4:27 AM · Aug 17, 2020 </span><span className="text-orange-700">Twitter for iPhone</span>
                        </div>
